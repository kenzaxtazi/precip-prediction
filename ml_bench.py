...
for station in hf_train_stations:
    ...
    station_ds = station_ds.expand_dims(dim={'lat': 1, 'lon':1})
...
# To arrays
hf_x_train = hf_train[['time', 'lon', 'lat']].values.reshape(-1,3)
...
lf_x_train = lf_train[['time', 'lon', 'lat']].values.reshape(-1,3)
lf_y_train = lf_train['tp'].values.reshape(-1,1)

x_val = val_df[['time', 'lon', 'lat']].values.reshape(-1,3)
...
def log_gp(x_train_h, y_train_h):
    kernel = GPy.kern.StdPeriodic(1, period=1) * GPy.kern.RBF(1) + GPy.kern.RBF(3)
