...
# create a CRS for the Cranfield model - parameters from file "All_EBands_NoEndorreic.prj"
cranfield_cs = iris.coord_systems.GeogCS(semi_major_axis=6377276.345, inverse_flattening=300.8017)
...
cr_xyz = cranfield_crs.transform_points(src_crs=ccrs.PlateCarree(), x=xb2, y=yb2)
crx = cr_xyz[:,:,0]
cry = cr_xyz[:,:,1]
...
swat_reader = shpreader.Reader('Data/Shapefiles/beas-sutlej-shapefile/12500Ha.shp')

# read the subbasin-to-basin map
map = pd.read_csv('Data/Shapefiles/beas-sutlej-shapefile/SWAT_subbasin_basins.csv')
...
for x in range(0, nx-1):
    for y in range(0, ny-1):
        polygon = Polygon([(crx[y,x], cry[y,x]), (crx[y+1,x], cry[y+1,x]),
                            (crx[y+1,x+1], cry[y+1,x+1]), (crx[y,x+1], cry[y,x+1])])
        if polygon.is_valid and envelope.intersects(polygon):
            overlap['Overlap'][y,x] = polygon.intersection(poly).area / polygon.area

# if any values are fractionally above 1, make them 1
overlap['Overlap'].where((abs(overlap['Overlap'] - 1) > 1e-05 + 1e-08 * overlap['Overlap']) | (overlap['Overlap']< 1), 1)
