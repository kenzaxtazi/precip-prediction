    ...
    for i in range(12):
        ...
        for b in list(combined_df):
            ...
            if kernel_density == True:
                ...
                axs[x, y].hist((grouped_dfs[i])[b], density=True, label=b, bins=np.arange(0, int(combined_df.max().max()))) #alpha=.5)
        axs[x, y].set_title(month_dict[i])
        axs[x, y].set_title(month_dict[i])
        axs[x, y].xaxis.set_tick_params(which="both", labelbottom=True)
        axs[x, y].yaxis.set_tick_params(which="both", labelbottom=True)
    ...
    for i in range(12):
        ...
        for b in list(combined_df):
            ...
            if kernel_density == True:
                ...
                axs[x, y].hist((grouped_dfs[i])[b], cumulative=True, density=True, label=b)
...
def mult_gauge_loc_plot(gauge_ds, other_datasets):
    ...
    combined_df["time"] = np.ceil((combined_df["time"] - np.floor(combined_df["time"])) * 12)
    ...
    for i in range(12):
        ...
        sns.kdeplot(data=month_list[i].drop(['Gauge data'], axis=1), linestyle='--', ax=axs[x, y], bw_adjust=1, legend=leg, common_norm=False)
        sns.kdeplot(data=month_list[i]['Gauge data'], c='k', ax=axs[x, y], bw_adjust=1, legend=leg)
